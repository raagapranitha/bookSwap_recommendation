...
'''
Reading the required columns from files
'''
print("Reading data")
users_cols = ['user_id']
users = pd.read_csv('data/user_info', sep='|', names=users_cols, encoding='latin-1')

# isbn_cols =['isbn','title']
# book_isbn = pd.read_csv('data/books.csv', sep=",", names=isbn_cols, encoding= 'latin-1' )
...
# proc_isbn=[]
# for name in books['title']:
#   df = book_isbn[book_isbn['title'].astype(str).str.contains(name)]
#   isbn = df.iloc[0]['isbn']
#   proc_isbn.append(isbn)
# books['isbn'] = proc_isbn
...
def split_dataframe(df, holdout_fraction=0.001):
...
reg_model = build_regularized_model(
    ...
    init_stddev=.05)
reg_model.train(num_iterations=10, learning_rate=20.)
